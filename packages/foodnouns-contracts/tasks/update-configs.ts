...
task('update-configs', 'Write the deployed addresses to the SDK and subgraph configs')
  ...
  .setAction(
    async (
      ...
      const sdkPath = join(__dirname, '../../foodnouns-sdk');
      ...
      try {
        ...
        console.log('Failed to re-build `@foodnouns/sdk`. Please rebuild manually.');
      }
      console.log('Addresses written to the Nouns SDK.');

      // Generate subgraph config
      const configName = `${network}-fork`;
      const subgraphConfigPath = join(__dirname, `../../foodnouns-subgraph/config/${configName}.json`);
